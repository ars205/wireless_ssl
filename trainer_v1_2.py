...
def prep_data_load(args):
    ...
    X_train, x_test, Y_train, y_test = train_test_split(csi2[:,:,0:100,:], tx_transform, stratify=tx_transform[:,2:3], test_size=500) #locations_ID2 was replaced by tx...
...
class Loss(nn.Module):
    ...
    def forward(self, targ_encoder, online_encoder, online_encoder_output, targ_encoder_output, epoch, it):
        ...
        Cross-entropy.
        """
        # targ_encoder centering and sharpening
...
##########################################################
    lr_schedule = logger.cosine_scheduler_lr(
    ...
    wd_schedule = logger.cosine_decay_scheduler_with_reset(
        args.weight_decay,
    ...
    momentum_schedule = logger.cosine_decay_scheduler_with_reset(args.momentum_targ_encoder, 1, args.epochs, len(data_loader))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--momentum_targ_encoder', default=0.994, type=float, help="""Base EMA.""")
    ...
    parser.add_argument('--weight_decay_end', type=float, default=0.5, help="""Final value of for the weight decay. Use cosine scheduler.""")
    parser.add_argument('--clip_grad', type=float, default=0.0, help="""Gradient norm if using gradient cropping.""")
    ...
    parser.add_argument('--saveckp_freq', default=2, type=int, help='Frequency of saving the models.')
    ...
    print('\n',transformation_global_1)
    print('\n',transformation_global_2)
    print('\n',transformation_ns)
