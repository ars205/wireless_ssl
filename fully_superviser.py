...
def train_eval_loop(model, train_loader, val_loader, loss_fn, optimizer, project_path, device, global_transform, num_epochs=1800, best_vloss = 0.0009, CHECKPOINT_PATH='./', experiment_name='Test'):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % 75 == 0:
            model_name_epoch = 'model_fully_supervisor_{}_{}'.format(timestamp, epoch_number)
...
############ When Using SSL and Linear Eval or Fine Tune or Fully-Supervised #################
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights_pth', type=str, default=CHECKPOINT_PATH+'/RANDOM.pth', help="Path to load pre-trained weights. Write here the name of the model (or Random for fully supervised).")
    ...
    ##### 5. Instatiate the FullySuperviserWrapper
    model_new = FullySuperviserWrapper(model,linear_regressor)     
    model_new.to(device)
    ...
    for ff in range(len(data_regimes)):
        ...
        #model.get_intermediate_layers
        print(f"\nArg weights jane:{args.weights_pth}\n")
        ### 0. Now load the pre-trained weights. ####
        logger.load_pretrained_weights(model, args.weights_pth, args.encoder)
        ...
        model_new = FullySuperviserWrapper(model, linear_regressor)     
        model_new.to(device)
        for layer in model_new.children():
        ...
        if data_regimes[ff].startswith('1k'):
            out_of_train_samples = 22001 # 22985, (kul:22001) (s:63212)this is for validation data, so that x_train is 1k
            train_and_val_batchsize = 128 # 
        elif data_regimes[ff].startswith('5k'):
            out_of_train_samples = 18001 # (s:59212) (kul:18001) (h:71200) this is for validation data, so that x_train is 5k
            train_and_val_batchsize = 512
        elif data_regimes[ff].startswith('10k'):
            out_of_train_samples = 13001 # (s:54212) (kul:13001) this is for validation data, so that x_train is 10k
            train_and_val_batchsize = 512    
        ...
        with open(config['project_path']+'/runs/trainig_stats_{}'.format(data_regimes[ff]+args.experiment_name), 'rb') as f:
            ...
            plot_stats(training_stats, results_path, save_results, name='FullySupervised_'+data_regimes[ff]+"_loss_curve", figsize=(10, 6))
        ...
        plot_results_actual_vs_estimate_ssl(actual_coordinates, estimated_coordinates, results_path, args.experiment_name, save=save_results, plot_name = 'FullySupervised_'+data_regimes[ff]+"_Actual_vs_Estimated")

        plot_results_ecdf(listMAE, results_path, args.experiment_name, save=save_results, error_type = 'FullySupervised_'+data_regimes[ff]+"_MAE")

        print_table_results_ssl(str(args.weights_pth)[-19:], transformation_type, args.experiment_name, listMAE, listRMSE, results_path, save=save_results, name_summarized = 'FullySupervised_'+data_regimes[ff]+'_summarized_res')

        if save_results == True:
            print("\n### Save Errors for ECDF at a later time.####")
            hf = h5.File(results_path+'/FullySupervised_'+data_regimes[ff]+"_MAE_errors_logs.h5", 'w')
            hf.create_dataset('listMAE', data=listMAE)
            hf.close()

            print("\n### Save Errors for ECDF at a later time.####")
            hf = h5.File(results_path+'/FullySupervised_'+data_regimes[ff]+"_RMSE_errors_logs.h5", 'w')
